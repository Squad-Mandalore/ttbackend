...
mod tests {
    ...
    fn test_iterate_hash_length() {
        ...
        const KEYCHAIN_NUMBER: i32 = 187;

        // when iterate_hash is called
        let hashed_password = iterate_hash(password, KEYCHAIN_NUMBER);
    ...
    fn test_iterate_hash_iterations() {
        ...
        let password = String::from("catgirls123");
        const KEYCHAIN_NUMBER: i32 = 187;
        const ANOTHER_KEYCHAIN_NUMBER: i32 = 361;

        // when iterate_hash is called with both keychain_numbers
        let hashed_password = iterate_hash(password.clone(), KEYCHAIN_NUMBER);
        let another_hashed_password = iterate_hash(password, ANOTHER_KEYCHAIN_NUMBER);
    ...
    async fn test_update_password(pool: PgPool) -> sqlx::Result<()> {
        ...
        let salt = get_salt(&pool, &employee_id).await?;
        assert!(salt.is_some());
        assert_eq!(dotenvy::var("SALT_LENGTH").expect("SALT_LENGTH variable is not set.").parse::<usize>().unwrap(), salt.unwrap().len());
